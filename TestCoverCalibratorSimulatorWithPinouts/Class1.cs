...
{
    public class TestCoverCalibratorSimulatorWithPinouts : RaspberryPi3Driver, ICoverCalibratorV1, IAlpacaDevice, ISimulation
    {
        ...
        {
            return (long)(someMagicNumber / stepsPerRevolution / speed);
        }

        public GpioController gpio;

        public List<int> stepperPins = new List<int> { 17, 22, 27, 23 }; // in1, in3, in2, in4.
                                                                         // in1 = a+, in3 = b+, in2 = a-, in4 = b-
        // a+ b+ a- b- per the datasheet https://www.omc-stepperonline.com/download/17HS19-2004S1.pdf
        public List<List<int>> phaseStep = new List<List<int>> {
        ...
        public TestCoverCalibratorSimulatorWithPinouts(int deviceNumber, ILogger logger, IProfile profile)
        {
            try
            {
                DeviceNumber = deviceNumber;
                gpio = new GpioController();
                gpio?.OpenPin(ledHotPin, PinMode.Output);

                foreach(var i in stepperPins)
                {
                    gpio.OpenPin(i, PinMode.Output);
                    gpio.Write(i, PinValue.Low);
                }
        ...
        { 
            if(gpio == null)
            {
                TL.LogError("No valid gpio");
                return;
            }
            gpio.Write(ledHotPin, lightEnabled ? PinValue.High : PinValue.Low);
            lightEnabled = !lightEnabled;
        }
        private void Spin(int numSteps)
        {
            if(numSteps == 0)
            {
                return;
        ...
        private void TakeStep(bool clockwise)
        {
            if (clockwise)
            {
                phaseIndex++;
                phaseIndex %= stepperPins.Count;
            }
            else
            {
                phaseIndex--;
                if(phaseIndex < 0)
                {
                    phaseIndex = stepperPins.Count - 1;
                }
            }
            for(int i = 0; i < phaseStep[phaseIndex].Count; ++i)
            {
                gpio.Write(stepperPins[i], phaseStep[phaseIndex][i]);
            }
        }
        
        private void Stop()
        {
            for (int i = 0; i < phaseStep[0].Count; ++i)
            {
                gpio.Write(stepperPins[i], 0);
            }
        }
